...
Every component page returned a `500`, and the status codes flapped like a broken traffic light:
...
I misread that pattern for a while. The `404` in the second row is the first request against a cold isolate, where a later lookup failed differently.
...
The defect was not in that file. That file was the first place the damage became visible. It shipped no change that week.
...
SSR that breaks in code you did not change is the tell.
...
The build exit code said `0` while the log held 194 server errors. The dependency tree said `vue`, singular, and meant two copies. My verification passed because I made cold requests, and my monitors stayed green because they never asked twice. So I read the error count, not the exit code:
